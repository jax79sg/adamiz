import click
fake_mixup_ratio = 0.0
min_mmd_dis = 99999.99

@click.command()
@click.option('--category', type=click.Choice([
    'atod', 'atow', 'dtoa', 'dtow', 'wtoa', 'wtod',
    ]), default='atod', help='experiment to run')
@click.option('--arch', type=click.Choice([
    '',
    'resnet50', 'resnet101', 'resnet152',
]), default='resnet50', help='network architecture')
...
@click.option('--gpu_id', type=int, default=0, help='Device')
@click.option('--num_threads', type=int, default=2, help='Number of worker threads')
@click.option('--skip_training',is_flag=True,default=False)
@click.option('--adamix', is_flag=True)



def experiment(category, arch, rnd_init, img_size, confidence_thresh, teacher_alpha, unsup_weight,
               ...
               gpu_id, num_threads,skip_training, adamix):
    settings = locals().copy()

    print('adamix is turned '+ str(adamix))
    if rnd_init:
        if fix_layers != '':
    ...
    import sys
    import pickle
    from imageDA import cmdline_helpers

    fix_layers = [lyr.strip() for lyr in fix_layers.split(',')]

    if log_file == '':
        log_file = 'output_aug_log_{}.txt'.format(category)
    elif log_file == 'none':
        log_file = None
    ...
    from imageDA import visda17_dataset
    from imageDA import office_dataset
    from imageDA import image_dataset
    from imageDA import network_architectures
    from imageDA import augmentation
    from imageDA import image_transforms
    import torch, torch.cuda
    from torch import nn
    from torch.nn import functional as F
    from imageDA import optim_weight_ema
    if adamix:
        import optim_weight_ema
        from mmd import diff_mmd
    ...
    with torch.cuda.device(gpu_id):
        ...
        if arch == '':
            if category in {'train_val', 'train_test'}:
                arch = 'resnet50'
        ...
        if category == 'visda_train_val':
        ...
        elif category == 'visda_train_test':
            d_source = visda17_dataset.TrainDataset(img_size=img_shape, range01=True, rgb_order=True)
            ...
            if adamix:
                d_src2tgt = office_dataset.OfficeAmazon2DslrDataset(img_size=img_shape, range01=True, rgb_order=True)
        elif category == 'atow':
            d_source = office_dataset.OfficeAmazonDataset(img_size=img_shape, range01=True, rgb_order=True)
            d_target = office_dataset.OfficeWebcamDataset(img_size=img_shape, range01=True, rgb_order=True)
            if adamix:
                d_src2tgt = office_dataset.OfficeAmazon2WebcamDataset(img_size=img_shape, range01=True, rgb_order=True)
        elif category == 'dtoa':
            ...
            if adamix:
                d_src2tgt = office_dataset.OfficeDslr2AmazonDataset(img_size=img_shape, range01=True, rgb_order=True)
        elif category == 'dtow':
            d_source = office_dataset.OfficeDSLRDataset(img_size=img_shape, range01=True, rgb_order=True)
            d_target = office_dataset.OfficeWebcamDataset(img_size=img_shape, range01=True, rgb_order=True)
            if adamix:
                d_src2tgt = office_dataset.OfficeDslr2WebcamDataset(img_size=img_shape, range01=True, rgb_order=True)
        elif category == 'wtoa':
            d_source = office_dataset.OfficeWebcamDataset(img_size=img_shape, range01=True, rgb_order=True)
            d_target = office_dataset.OfficeAmazonDataset(img_size=img_shape, range01=True, rgb_order=True)
            if adamix:
                d_src2tgt = office_dataset.OfficeWebcam2AmazonDataset(img_size=img_shape, range01=True, rgb_order=True)
        elif category == 'wtod':
            d_source = office_dataset.OfficeWebcamDataset(img_size=img_shape, range01=True, rgb_order=True)
            d_target = office_dataset.OfficeDSLRDataset(img_size=img_shape, range01=True, rgb_order=True)
            if adamix:
                d_src2tgt = office_dataset.OfficeWebcam2DslrDataset(img_size=img_shape, range01=True, rgb_order=True)
        else:
            print('Unknown experiment type \'{}\''.format(category))
            return
        ...
        #summary(student_net.cuda(), (3,224,224))
        ...
        if adamix:
            if constrain_crop >= 0:
                def augment_mixup(X_sup,X_fake_sup, y_sup, X_tgt ):
                    X_sup = sup_xf(X_sup)[0]
                    X_fake_sup = sup_xf(X_fake_sup)[0]
                    X_mix = fake_mixup_ratio * np.asarray(X_fake_sup) + (1. - fake_mixup_ratio) * np.asarray(X_sup)
                    X_unsup_both = unsup_xf(X_tgt)[0]
                    X_unsup_stu = X_unsup_both[:len(X_tgt)]
                    X_unsup_tea = X_unsup_both[len(X_tgt):]

                    return X_mix, y_sup, X_unsup_stu, X_unsup_tea
            else:
                def augment_mixup(X_sup, X_fake_sup, y_sup, X_tgt):
                    X_sup = sup_xf(X_sup)[0]
                    X_fake_sup = sup_xf(X_fake_sup)[0]
        ...
        def f_train(X_sup, y_sup, X_unsup0, X_unsup1):
            ...
            sup_logits_out,_ = student_net(X_sup)
            student_unsup_logits_out, _ = student_net(X_unsup0)
            student_unsup_prob_out = F.softmax(student_unsup_logits_out, dim=1)

            teacher_unsup_logits_out, unsup1_feats  = teacher_net(X_unsup1)
            teacher_unsup_prob_out = F.softmax(teacher_unsup_logits_out, dim=1)
            if adamix:
                _, sup_feats = teacher_net(X_sup)

            # """ CAUTION: will be averaged by number of samples in batch_up_mean lib ! """
            # mean_dis_b = torch.mean(torch.abs(sup_feats - unsup1_feats)).data.cpu().numpy()
                mmd_dis_b = diff_mmd(sup_feats.data, unsup1_feats.data).cpu().numpy()
            # swd_dis_b = dis_swd_torch(sup_feats.data, unsup1_feats.data).cpu().numpy()
            # """ ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ """
                """ ~~~~~~~~~~~~~~ CAUTION: change mix ratio every iter ~~~~~~~~~~~~~~ """
                min_mmd_dis = min(min_mmd_dis, mmd_dis_b)
                fake_mixup_ratio = min_mmd_dis / mmd_dis_b * float(epoch) / float(num_epochs)
            """ ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ """


            # Supervised classification loss
            if double_softmax:
                clf_loss = classification_criterion(F.softmax(sup_logits_out, dim=1), y_sup)
            else:
            ...
            mask_count = conf_mask_count.data.cpu().item()

            if adamix:
                outputs = [float(clf_loss.data.cpu().item()) * n_samples,
                           float(unsup_loss.data.cpu().item()) * n_samples,
                           mask_count,
                           mmd_dis_b*n_samples]
        ...
        def f_pred_src(X_sup):
            ...
            return (F.softmax(teacher_net(X_var), dim=1).data.cpu().numpy(),)

        def f_pred_tgt(X_sup):
            X_var = torch.autograd.Variable(torch.from_numpy(X_sup).cuda())
            teacher_net.train(mode=False)
            if adamix:
                return (F.softmax(teacher_net(X_var)[0], dim=1).data.cpu().numpy(),)
            return (F.softmax(teacher_net(X_var)[0], dim=1).data.cpu().numpy(),)
        ...
        def f_pred_tgt_mult(X_sup):
            ...
            for aug_i in range(len(X_sup)):
                ...
                y_pred = F.softmax(teacher_net(X_var)[0], dim=1).data.cpu().numpy() if adamix else F.softmax(teacher_net(X_var), dim=1).data.cpu().numpy()
        ...
        if adamix:
            # ------------------------------------------------------------------------
            sup_mix_ds = data_source.ArrayDataSource([d_source.images, d_src2tgt.images, d_source.y], repeats=-1, indices=source_indices)
            tgt_train_ds = data_source.ArrayDataSource([d_target.images], repeats=-1, indices=target_indices)
            train_mix_ds = data_source.CompositeDataSource([sup_mix_ds, tgt_train_ds]).map(augment_mixup)
            train_mix_ds = pool.parallel_data_source(train_mix_ds, batch_buffer_size=min(20, n_train_batches))
            # ----------------------------------------------------------------------------------------------------------
        else:
        ...
        train_batch_iter = train_mix_ds.batch_iterator(batch_size=batch_size, shuffle=shuffle_rng) if adamix else train_ds.batch_iterator(batch_size=batch_size, shuffle=shuffle_rng)
        if not skip_training:
            mmds, swds = ([] for i in range(2))
            for epoch in range(num_epochs):
                ...
                if adamix:
                    train_clf_loss, train_unsup_loss, mask_rate, mmd_dis_ep = data_source.batch_map_mean(
                        f_train, train_batch_iter, progress_iter_func=progress_bar, n_batches=n_train_batches)
                    mmds.append(mmd_dis_ep)
                else:
