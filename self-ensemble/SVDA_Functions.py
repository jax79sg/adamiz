...
class EMAWeightOptimizer (object):
    ...
    def step(self):
        ...
        for tgt_p, src_p in zip(self.target_params, self.source_params):
            #print(tgt_p.type(torch.FloatTensor).type())
            tgt_p.type(torch.cuda.FloatTensor).mul_(self.ema_alpha)
            tgt_p.type(torch.cuda.FloatTensor).add_(src_p * one_minus_alpha)
