...
def train():
    ...
    for epoch in range(n_epoch):
        ...
        while i < len_dataloader+1:
            ...
            #if (category=="stom") or (category=="mtou"):
            err_s_label = loss_class(train_class_output, s_label)
            #else: err_s_label = loss_class(train_class_output,torch.max(s_label, 1)[0])
            ...
            #else: n_train_correct += (pred == torch.max(s_label,1)[0]).sum().item()
        ...
        if epoch % 1 == 0:
            ...
            for idx, sample_val in enumerate(SourceTestDataLoader, 0):
                ...
                with torch.no_grad():
                    ...
                    #if category =="utom":
                    #  n_correct_stu += (pred == torch.max(label_val,1)[0]).sum().item()
                    #  class_output_tea = net_tea(img_val)
                    #  pred = class_output_tea.max(1)[1]
                    #  n_correct_tea += (pred == torch.max(label_val,1)[0]).sum().item()
                    #else:
        ...
        if epoch % 1 == 0:
            ...
            for it, sample_t in enumerate(TargetTestDataLoader, 0):
                ...
                with torch.no_grad():
                    ...
                    #if category =="mtou":
                    #  n_target_correct_stu += (pred == torch.max(label_test,1)[0]).sum().item()
                    #  class_output_tea = net_tea(img_test)
                    #  pred = class_output_tea.max(1)[1]
                    #  n_target_correct_tea += (pred == torch.max(label_test,1)[0]).sum().item()
                    #else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--n_epoch', type=int, default=200, help="batch_size")
    ...
    if category =="stom":
      summary(net_tea.cuda(), (3,32,32))
    else: summary(net_tea.cuda(),(1,28,28))
